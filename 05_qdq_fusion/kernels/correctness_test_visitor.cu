// Stage 5 visitor-path correctness: Sm90 INT8 GEMM + LinCombEltAct
// Fast-GELU/quantize vs the same CPU affine reference as the SIMT path.
// Skips (exit 0) when the GPU compute capability is < 90.
...
void Reference(int M, int N, int K, const std::vector<int8_t> &A,
  ...
  for (int m = 0; m < M; ++m) {
    for (int n = 0; n < N; ++n) {
      ...
      for (int k = 0; k < K; ++k) {
        double a =
            (static_cast<int>(A[m * K + k]) - za) * static_cast<double>(sa);
        double b =
            (static_cast<int>(B[k * N + n]) - zb) * static_cast<double>(sb);
        acc += a * b;
      }
...
bool RunCase(int M, int N, int K) {
  ...
  bool ok = QgemmBiasGeluInt8VisitorSm90(M, N, K, dA, dB, dBias, dD, sa, za, sb,
                                         zb, sd, zd, nullptr);
  ...
  std::printf(
      "[qgemm_bias_gelu_visitor_sm90] M=%d N=%d K=%d launched=%d max_abs=%d "
      "mismatches=%d -> %s\n",
