...
int8_t QuantizeD(float y, float scale_d, int32_t zp_d) {
  return QuantizeAffine(y, scale_d, zp_d);
}
...
bool RunCase(int M, int N, int K) {
  ...
  for (size_t i = 0; i < fA.size(); ++i)
    fA[i] = static_cast<float>((static_cast<int>(i) % 17) - 8) * 0.05f;
  for (size_t i = 0; i < fB.size(); ++i)
    fB[i] = static_cast<float>((static_cast<int>(i) % 13) - 6) * 0.04f;

  const float sa = 0.05f, sb = 0.04f, sd = 0.08f;
  const int32_t za = 1, zb = -2, zd = 3;

  std::vector<int8_t> hA, hB;
  QuantizeAffineTensor(fA, hA, sa, za);
  QuantizeAffineTensor(fB, hB, sb, zb);
  ...
  std::printf(
      "[qgemm_bias_gelu] M=%d N=%d K=%d launched=%d max_abs=%d mismatches=%d "
      "-> %s\n",
